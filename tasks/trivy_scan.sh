#!/bin/sh
# stagehand::trivy_scan — run Trivy on a node, normalize to compliance.v1 with the
...
# Single combined EXIT trap for both the report file (set below, once REPORT
# is assigned) and the pinned-install temp dir (set inside the install branch,
# if it runs). Declared once, up front, so neither cleanup clobbers the other
# via a second `trap ... EXIT` later in the script.
REPORT=""
TMPDIR=""
trap 'rm -f "${REPORT:-}"; rm -rf "${TMPDIR:-}"' EXIT
...
ADAPTER="${INSTALLDIR}/stagehand/files/trivy-report.sh"
